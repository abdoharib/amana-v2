...
const props = defineProps({
  ...
  prayerContent: Object, // The prayer being edited
  errors: {
    required: true,
  },
  prayerId: {
...
watch(() => props.prayerContent, newPrayer => {
  if (newPrayer) {    
...
const onSubmit = () => {
  refForm.value?.validate().then(({ valid }) => {
    if (valid) {
      ...
      if(selectedFile.value) formData.append("audio", selectedFile.value);
      formData.append("content", form.value.content);
      formData.append("prayer_id", props.prayerId);

      emit('prayerContentData', formData);
        ...
        <VCardText>
          ...
          <VForm ref="refForm" v-model="isFormValid" @submit.prevent="onSubmit">
            <VRow>
              <!-- 👉 Title -->
              <VCol cols="12">
                <AppTextarea v-model="form.content" :rules="[requiredValidator]" label="الذكر"
                  placeholder="أدخل الذكر" />
              </VCol>

              <!-- 👉 Image Path -->
              <VCol cols="12">
                <VFileInput label="تحميل صوت الذكر" @change="handleFileChange" />
              </VCol>
