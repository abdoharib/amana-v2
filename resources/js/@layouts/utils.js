...
export const isNavLinkActive = (link, router) => {
  ...
  const currentParamsType = router.currentRoute.value.params.type; // Get current type

  const resolveRoutedName = resolveNavLinkRouteName(link, router);
  if (!resolveRoutedName) return false;

  return matchedRoutes.some(route => {
    // Ensure both name and type match
    return (
      route.name === resolveRoutedName ||
      route.meta.navActiveLink === resolveRoutedName
    ) && link.to.params?.type === currentParamsType; // Compare params.type
  });
};
